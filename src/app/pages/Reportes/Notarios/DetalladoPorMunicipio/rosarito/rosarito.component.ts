...
@Component({
  selector: 'ngx-rosarito',
  templateUrl: './rosarito.component.html',
  styleUrls: ['./rosarito.component.scss'],
})
export class RosaritoComponent implements OnInit {
  ...
  obtenerDatos() {
    ...
    this.objServicio.objNotariosController.DetalladoPorMunicipio(parametros).pipe(first()).subscribe((respuestaServer: any) => {
      ...
      this.tblDatos.forEach(value => {
        // Validar que la notaria no contenga el dato 14,
        // ya que esto representa el total y no queremos que se muestre en la grafica como barra.
        if (value.NOTARIA !== 14 && value.NOTARIA !== 0) {
